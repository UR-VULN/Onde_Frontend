...
// 🛡️ 누락되었던 임포트 추가
import { getMemberRole, getMemberId } from '@/utils/authCookies';
...
adminAxios.interceptors.request.use(
  (config) => {
    // 서버가 발행한 XSRF-TOKEN 쿠키를 읽어옴
    const csrfToken = Cookies.get('XSRF-TOKEN');
    
    // Axios는 기본적으로 X-XSRF-TOKEN 헤더를 통해 CSRF 토큰을 전달
    if (csrfToken) {
...
export const get_admin_dashboard_api = async (
  ...
  // 🛡️ 에러를 해결하기 위해 쿠키에서 직접 role을 불러옵니다.
